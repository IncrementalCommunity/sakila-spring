package web.sakila.webapi.controller;
...
import web.sakila.webapi.model.StoreWO;
import web.sakila.webapi.service.StoreService;
...
public class StoreRestController {
	...
	public ResponseEntity<StoreWO> updateStore(@RequestBody StoreWO storeWO, UriComponentsBuilder ucBuilder) {
		...
		log.debug(String.format("Updating store %s ", storeWO.getStoreId()));
		
		StoreWO currentStore = storeService.findById(storeWO.getStoreId());

		if (currentStore == null) {
			System.out.println("Store with id " + storeWO.getStoreId() + " not found");
			return new ResponseEntity<StoreWO>(HttpStatus.NOT_FOUND);
		}
	...
	public ResponseEntity<StoreWO> deleteStore(@PathVariable("id") int id) {
		...
		log.debug("Fetching & Deleting Store with id " + id);
